...
abbrev N [Group G] : Rep.ofMulAction k G G ⟶ Rep.ofMulAction k G G :=
  Rep.ofHom <| (((Representation.ofMulAction k G G).asAlgebraHom
    ...
    rw [show (∑ i : G, MonoidAlgebra.single i (1 : k)) (g2 * g⁻¹) =
        ∑ i : G, MonoidAlgebra.single i (1 : k) (g2 * g⁻¹) from Finsupp.finsetSum_apply _ _ _,
      show (∑ i : G, MonoidAlgebra.single i (1 : k)) (g⁻¹ * g2) =
        ∑ i : G, MonoidAlgebra.single i (1 : k) (g⁻¹ * g2) from Finsupp.finsetSum_apply _ _ _]
    simp
...
abbrev π_aux [CommGroup G] : ((ChainComplexAbel G k σ).X Nat.zero) ⟶
    (((ChainComplex.single₀ (Rep k G)).obj (Rep.trivial k G k)).X Nat.zero) :=
...
def CyclicCoh.homotopy_aux [CommGroup G] : (Rep.trivial k G k) ⟶
    ((ChainComplexAbel G k σ).X 0) :=
...
lemma im_sigmainus1_eq_ker_π [CommGroup G] (hσ : Submonoid.powers σ = ⊤) :
  ...
  have hsig : ∀ y : MonoidAlgebra k G, (Rep.Hom.hom (sigmaminus1 G k σ)).toLinearMap y =
      (Representation.ofMulAction k G G).asAlgebraHom (MonoidAlgebra.single σ 1 - 1) y := fun _ => rfl
  simp only [hsig]
  erw [map_sum]
...
lemma N_ker_eq_im_sigmainus1 [CommGroup G] : LinearMap.ker (N G k).hom.toLinearMap =
    LinearMap.range (sigmaminus1 G k σ).hom.toLinearMap := SetLike.ext_iff.2 fun (x : MonoidAlgebra k G) ↦
  ⟨fun hx ↦ by
    simp at hx ⊢
    sorry, by sorry⟩

lemma sigmaminus1_ker_eq_N_im [CommGroup G] :
    LinearMap.ker (sigmaminus1 G k σ).hom.toLinearMap = LinearMap.range (N G k).hom.toLinearMap := by
  sorry
...
def singletonVasRep [Group G] (V : ModuleCat k) [Subsingleton V] : CategoryTheory.Limits.IsZero
  ...
  unique_from W := ⟨⟨⟨Rep.ofHom ⟨0, fun _ ↦ by ext; simp⟩⟩, fun f ↦ by
    ext x
    simp only [Rep.hom_ofHom, Representation.IntertwiningMap.coe_mk, LinearMap.zero_apply]
    ...
    | zero =>
      ...
      · intro a _ x hx
        ...
        simp only [Functor.mapShortComplex_obj, ShortComplex.map_X₁, shortComplexFunctor'_obj_X₁,
          ShortComplex.map_X₂, shortComplexFunctor'_obj_X₂, ChainComplex.single₀_obj_zero,
          ShortComplex.map_f, shortComplexFunctor'_obj_f, single_obj_d, Functor.map_zero,
          ModuleCat.hom_zero, LinearMap.zero_apply, Functor.mapShortComplex_map_τ₂,
          shortComplexFunctor'_map_τ₂, Rep.forget₂_moduleCat_map, ModuleCat.hom_ofHom] at hx
        exact hx.symm
      · intro (a : k) _
        refine ⟨Finsupp.single 1 a, ?_, 0, ?_⟩
        · show ((forget₂ (Rep k G) (ModuleCat k)).mapShortComplex.obj
            ((shortComplexFunctor' (Rep k G) (ComplexShape.down ℕ) 1 0 0).obj
              (ChainComplexAbel G k σ))).g.hom (Finsupp.single 1 a) = 0
          rfl
        · rw [map_zero, eq_comm, sub_eq_zero]
          change ((π G k σ).f 0).hom.toLinearMap (Finsupp.single 1 a) = a
          simp only [π, π_aux, Nat.rec_zero, Rep.hom_ofHom]
...
abbrev equiv_Acomplex [CommGroup G] (A : Rep k G) : (ChainComplexAbel G k σ).linearYonedaObj k A ≅
  ...
  · ext (f : Rep.ofMulAction k G G ⟶ A)
    show (N' G k A).hom.toLinearMap (A.leftRegularHomEquiv f) =
      A.leftRegularHomEquiv (Linear.leftComp k A (N G k) f)
    show (N' G k A).hom.toLinearMap (f.hom.toLinearMap (Finsupp.single (1 : G) 1)) =
    ...
    show A.ρ.asAlgebraHom (∑ i : G, .single i 1) (f.hom.toLinearMap (Finsupp.single (1 : G) 1)) =
...
abbrev CyclicCoh.groupCohEven (hn : Even n) [h : NeZero n] [CommGroup G] [DecidableEq G]
  ...
  (by
    ...
    have hker : ∀ y : A.V, y ∈ (ModuleCat.Hom.hom
      ...
      intro y
      show (sigmaminus1' G k σ A).hom.toLinearMap y = 0 ↔ _
    ...
    (by
      ...
      · have hle : LinearMap.range (N' G k A).hom.toLinearMap ≤ A.ρ.invariants := by
        ...
        simp only [Acomplex, HomologicalComplex.shortComplexFunctor_obj_X₁,
          ...
          CochainComplex.prev_nat_succ, ↓reduceIte, hn, forget₂_map_hom, ModuleCat.hom_ofHom]
...
abbrev CyclicCoh.groupCohOdd (hn : Odd n) [h : NeZero n] [CommGroup G] [DecidableEq G]
  ...
  (by
    ...
    | succ m =>
      ...
      simp only [Acomplex, HomologicalComplex.shortComplexFunctor_obj_X₁,
        HomologicalComplex.shortComplexFunctor_obj_f, Functor.mapHomologicalComplex_obj_d,
        CochainComplex.prev_nat_succ, ↓reduceIte,
